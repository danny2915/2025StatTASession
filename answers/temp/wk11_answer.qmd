...
::: {.callout-tip title="Hint"}
For each row in `shifts_coverage`, create a temperate data frame `temp` in the for-loop with columns `name`, `plot`, `date`, where `date` columns includes all the dates between `start_date` and `end_date`. 
:::
...
shifts <- read.csv(here("data", "garden_shifts.csv"))
